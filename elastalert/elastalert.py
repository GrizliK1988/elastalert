...
class ElastAlerter():
    ...
    def __init__(self, args):
        ...
        self.writeback_index_prefix = self.conf['writeback_index_prefix']
    ...
    def get_starttime(self, rule):
        ...
        try:
            res = self.writeback_es.search(index=self.writeback_index_prefix + 'elastalert_status', doc_type='doc',
    ...
    def wait_until_responsive(self, timeout, clock=timeit.default_timer):
        ...
        while (clock() - ref) < timeout:
            try:
                if self.writeback_es.indices.exists(self.writeback_index_prefix + 'elastalert_status'):
                    return
        ...
        if self.writeback_es.ping():
            logging.error(
                ...
                self.writeback_index_prefix + 'elastalert_status',
            )
        else:
            logging.error(
    ...
    def writeback(self, doc_type, body):
        ...
        try:
            res = self.writeback_es.index(index=self.writeback_index_prefix + doc_type,
    ...
    def find_recent_pending_alerts(self, time_limit):
        ...
        query.update(sort)
        try:
            res = self.writeback_es.search(index=self.writeback_index_prefix + 'elastalert',
                                           doc_type='doc',
                                           body=query,
                                           size=1000)
            if res['hits']['hits']:
    ...
    def send_pending_alerts(self):
        ...
        for alert in pending_alerts:
            ...
            if ts_now() > ts_to_dt(alert_time):
                ...
                try:
                    self.writeback_es.delete(index=self.writeback_index_prefix + 'elastalert',
                                             doc_type='doc',
                                             id=_id)
    ...
    def is_silenced(self, rule_name):
        ...
        try:
            res = self.writeback_es.search(index=self.writeback_index_prefix + 'silence', doc_type='doc',
